 ...
 * es nur woanders?
 *
 * Deshalb ist `wirkung` das eigentliche Feld und die Farbe nur das
 * Etikett. Wer eine eigene Klasse anlegt, muss die Wirkung mit angeben —
 ...
 */
/**
 * Was eine Klassifizierung mit dem Geld macht. Drei Werte, nicht zwei:
 *
 *   verloren   das Geld ist weg          — Miete, Essen, Versicherung
 *   erhalten   liegt woanders            — Investition, Sparen, blockiert
 *   durchlauf  gehoert mir gar nicht     — zurueckgezahlte Auslagen, fremdes
 *                                          Geld auf meinem Konto
 *
 * Ein Wahrheitswert `verloren: true|false` reichte dafuer nicht. Durchlaufgeld
 * ist weder das eine noch das andere: es darf weder als Einkommen noch als
 * Ausgabe noch im Restwert auftauchen, muss aber sichtbar bleiben — sonst
 * sucht man jeden Monat, warum die Zahlen nicht stimmen.
 */
export const WIRKUNGEN = ["verloren", "erhalten", "durchlauf"];

export const WIRKUNG_TITEL = {
  verloren: "Das Geld ist weg",
  erhalten: "Liegt woanders",
  durchlauf: "Gehört mir nicht"
};

export const STANDARD_KLASSEN = [
  { id: "ausgaben",    name: "Ausgaben",              farbe: "rot",   wirkung: "verloren" },
  { id: "investition", name: "Investition",           farbe: "gruen", wirkung: "erhalten" },
  { id: "sparen",      name: "Sparen",                farbe: "gelb",  wirkung: "erhalten" },
  { id: "blockiert",   name: "Investition blockiert", farbe: "lila",  wirkung: "erhalten" },
  { id: "durchlauf",   name: "Durchlaufgeld",         farbe: "blau",  wirkung: "durchlauf" }
];
...
export function leseKlassen(roh) {
  ...
  for (const k of roh) {
    ...
    klassen.push({
      ...
      /* Eine unbekannte oder fehlende Wirkung gilt als „weg“ — die
         vorsichtigere der drei Annahmen. Der alte Wahrheitswert aus einer
         Zwischenfassung wird dabei mitgelesen. */
      wirkung: WIRKUNGEN.includes(k.wirkung)
        ? k.wirkung
        : (k.verloren === false ? "erhalten" : "verloren"),
